...
names= [
'证券名称', '证券代码', '信用评级', '修正久期（年）', '认可价值', '风险暴露', '基础因子RF0', '最低资本'
]
    ...
    try :
        ...
        for mem in members:
            ...
            for name_gid in result_name_gid:
                dim_gid = name_gid[1]
                li = []
                li.append(name_gid[0])
                li.append(name_gid[1])
                for item in items:
                    li.append(get_value_ele_dim(item, dim_gid, dwzd_bh, inst_date))
                print(li)
                write(li)
